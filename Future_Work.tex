...
Using a better SLAM/VO algorithm would be perhaps the easiest and cheapest way to improve the system. Testing out different vision algorithms would thus be an appealing avenue of inquiry. It would behoove the researcher to characterize the system's behavior under the same conditions which caused PTAM to fail. In particular, experiments that include pure rotations, longer translations, and motion over non-flat terrain would be of interest. Mounting the sensor suite on the underside of a real UAV would allow for collection of true flight data, perhaps exposing the limits of the system's limits at speed and in the presence of vehicle vibrations.

After integrating new sensors, new experiments could be devised to characterize the system's effectiveness during sensor blackouts---for example, during an indoor-outdoor transition after integrating a GPS receiver. Experiments such as this could then inform the design of heuristic models for sensor degradation and perhaps even consensus-based estimation strategies. 
...
Growing demand for so-called ``precision agriculture'' has brought with it the need to map large areas of cropland at a high speed and a low cost point. In the past, this need has been met using satellite imagery and human-captured photography. With the arrival of low-cost drones, this work has been offloaded to aerial robots. Outfitted with special sensors such as hyperspectral cameras\footnote{\url{https://en.wikipedia.org/wiki/Hyperspectral_imaging}}, small UAS are able to provide minute-by-minute coverage of large expanses of land. More interesting still, these robots can be outfitted with reservoirs of pesticide or fertilizer and deployed to spray individual plants autonomously. It is in roles such as this where high-accuracy robust localization becomes a necessity. The precision required for precision agriculture can be provided using the sensor fusion techniques discussed in this thesis.
