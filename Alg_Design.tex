...
\end{equation}
%
To calculate the vehicle's current acceleration, we first subtract gravity from the measured acceleration vector $\mathbf{a}_{k}$ and then rotate the difference into the inertial frame: \todo{Have Sam look over this}
%
\begin{equation}
\mathbf{a}_{k | k-1} = R^{g}_{b} \left( \mathbf{q}_{k-1} \right) \left( \mathbf{a}_{k} - \mathbf{g} \right) .
\end{equation}
%
...
\todo{velocity equation}

\todo{position equation}

\todo{ang vel equation}
